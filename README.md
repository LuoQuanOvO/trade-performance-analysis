...
> **基差口径**:本框架中基差 = 期货主力收盘价 - 现货价(与交易所/行业通行的"期货-现货"口径一致)。
> 基差为正 = 期货升水(现货贴水);基差为负 = 期货贴水(现货升水)。

| 指标 | 数值 | 解读 |
|------|------|------|
| 期货主力收盘 | 3166 元/吨 | 自8月中旬3016反弹至3166(+5%),价格回升 |
| 最新基差 | +15 元/吨 | 由8月中旬的-62(期货贴水62)转为期货升水,基差由贴水修复至平水上方 |
| 基差率 | +0.48% | 处于近180日96%分位(偏高)——期货升水处于历史高位区间 |
| **基差率历史分位** | **近180日96%分位** | 三个月前从86%→36%回落(期货贴水加深),本次重新升至96%(期货转升水),基差结构反复切换 |
...
**期现正套损益测算(买入现货+卖出期货,资金成本年化4.5%)**:正套盈亏 = 初始基差 − 目标基差 − 资金成本,
盈亏平衡基差 = 初始基差 − 资金成本。当前基差 +15(期货升水):
按保守的"收敛至平水"情景,30天净收益约 **+3元/吨**(资金成本约11.6元/吨),仅薄利;
按"收敛至样本均值"情景(近180日样本均值,以最新运行为准)测算空间更大,但依赖基差均值回归假设。
S2信号提示基差修复接近完成——反套离场区/正套观察。

**跟踪信号表**:以"基差分位 + 库存拐点 + 盘面利润"三维触发,
当前 S2(基差修复接近完成/反套离场)、S5增产压力已触发(S1基差修复观察、S3基差修复受阻未触发)。

**研究结论**:8月中旬至9月初框架捕捉到第二次市场状态切换——基差-62→+15(期货贴水转升水)、
分位36%→96%、库存加速累库(+51252吨/30日)但价格反弹(+150),典型"涨价与累库并存"背离结构,
反映资金与情绪推动期货走强,但基本面库存压力未解。**框架的价值在动态跟踪**:结论是动态的,
框架是常驻的——当前期货升水处于高位分位,保守情景下正套仅薄利(均值回归情景空间更大但依赖假设),
关注库存拐点与基差回落后的再平衡机会。
