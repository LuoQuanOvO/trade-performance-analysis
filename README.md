...
手续费绝对额下降93%;账户未发生爆仓(资金未归零)。口径定义见下节,改进不是赌出来的,而是归因+验证出来的。
...
| 价格-库存相关性 | r=0.23 | 相关性由0.49降至0.23(随数据更新,以最新运行为准),涨价与累库并存,相关关系明显弱化 |
...
# 复现历史数据截止日: AS_OF=20260904 python black_series.py
