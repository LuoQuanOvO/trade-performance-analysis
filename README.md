...
| 基差率 | -0.48% | 处于近180日4%分位(偏低)——期货升水处于历史高位区间 |
| **基差率历史分位** | **近180日4%分位** | 三个月前从14%→64%回升(期货贴水加深),本次重新降至4%(期货转升水),基差结构反复切换 |
...
按"收敛至样本均值"情景(近180日样本均值,以最新运行为准)测算空间更大,但依赖基差均值回归假设。
...
- 基差取自主力连续合约,换月时基差序列存在跳变,分位/均值为近似;严格期现套利应锚定具体交割月合约
